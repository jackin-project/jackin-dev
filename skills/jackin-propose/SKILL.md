---
name: jackin-propose
description: Opens a new jackin❯ feature or idea as a roadmap item draft plus an early pull request, without writing any code. Use when the operator runs /jackin-dev:jackin-propose.
...
- Operator runs `/jackin-dev:jackin-propose <idea or problem>`.
- Feature/idea worth a roadmap item.

## When NOT to use

- Small fix (typo, dep bump, one-liner) → `/jackin-dev:jackin-create-pr`.
- Designing an existing item → `/jackin-dev:jackin-brainstorm`. Implementing one → `/goal Implement <slug>.md`.
...
2. **(optional) Research.** On `--research`, run the built-in `deep-research` skill and summarize findings into the draft. For large research, prefer `/jackin-dev:jackin-research`.
...
6. **Stop.** Point the operator at the next step: `/jackin-dev:jackin-brainstorm <slug>` to fill `## Design`. Do not implement.
